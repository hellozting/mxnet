...
def plot_network(symbol, title="plot", shape=None, node_attrs={}):
    ...
    # color map
    cm = ("#8dd3c7", "#ff6666", "#ffff88", "#aaaadd", "#88bbff",
          "#fdb462", "#b3de69", "#fccde5", "#ffbbbb")
    cm_left = ("#ddeedd", "#ff5555", "#ffeeb0", "#c0cbeb", "#90c1ff",
          "#ffc4ff", "#c3eeff", "#ffaaff", "#aacbff")
    my_colors =('#4D4D4D','#5DA5DA','#FAA43A',
                '#a0DDa8','#F17CB0','#B2912F',
                '#B276B2','#DECF3F','#F0a8a4',)
    # my_colors
    all_colors = matplotlib.cm.rainbow(np.linspace(0, 1, 20))
    # cm=all_colors[:9]
    # cm_left=all_colors[10:]
    # make nodes
    ignore_nodes=["Activation_","BatchNorm_","Flatten","_MulScalar","Pooling","SoftmaxOutput"]
    conv_num=0
    global_conv_num=0
    conv_num_dict={}
    for i in range(len(nodes)):
        ...
        if op == "null":
            ...
            cur_color=cm_left[group_num] if is_left else cm[group_num]
            if 'uncouple' in name:    #uncoupled with shared layers
                ...
                conv_num+=0 if is_left and 'nonshare' not in name else 1      
            ...
            label= r"BN"
            attr["fillcolor"] = my_colors[3]
        elif op == "Activation" or op == "LeakyReLU":
            label = r"%s" % (node["param"]["act_type"])
            attr["fillcolor"] = my_colors[8]
        elif op == "Pooling":
            ...
            attr["fillcolor"] = my_colors[5]
        elif op == "Softmax":
            attr["fillcolor"] = my_colors[6]
        elif op == "SoftmaxOutput":
            label=r"Softmax"
            attr["fillcolor"] = my_colors[7]
        elif op == "_Plus":
            inputs = node["inputs"]
            for item in inputs:
